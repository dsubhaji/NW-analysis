...
if (choice==1){
  ...
  for(i in 1:len)
  {
    ...
    write.table(cbind(degree_in,degree_out,degree,betweenness,clustcoeff,closeness,eigencentrality,pagerank), 
                file1, row.names = FALSE,
                col.names=c('in_degree','out_degree','degree','betweenness','clustcoeff','closeness','eigencentrality','pagerank'),sep=",",na="0")
